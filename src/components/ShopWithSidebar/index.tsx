...
const ShopWithSidebar = () => {
  ...
  const categories = useMemo(() => {
    const cats = Array.from(new Set(shopData.map((p) => p.category)));
    return cats.map((cat) => ({
      name: cat.charAt(0).toUpperCase() + cat.slice(1),
      value: cat,
      products: shopData.filter((p) => p.category === cat).length,
    }));
  }, []);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !target.closest('button[aria-label="button for product sidebar toggle"]')
      ) {
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2.5">
                    <button
                      ...
                      } flex items-center justify-center w-10.5 h-9 rounded-[5px] border hover:bg-blue hover:border-blue hover:text-white transition`}
                    >
                      <svg
                        className="fill-current"
                        width="18"
                ...
                <div className="bg-white shadow-1 rounded-md p-2">
                  <ul className="flex items-center">
                    <li>
                      <button
                        disabled
                        className="flex items-center justify-center w-8 h-9 rounded-[3px] text-gray-4"
                      >
                        <svg
                          className="fill-current"
                          width="18"
